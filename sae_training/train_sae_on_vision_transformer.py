...
def train_sae_on_vision_transformer(
    ...
    if resume:
        ...
        act_freq_scores = torch.zeros(sparse_autoencoder.cfg.d_sae,
        ...
        if n_checkpoints > 0:
            checkpoint_thresholds = list(
                range(0, total_training_tokens,
                      total_training_tokens // n_checkpoints))[1:]
    ...
    while n_training_tokens < total_training_tokens:
        ...
        if (
                n_training_steps + 1
        ) % sparse_autoencoder.cfg.feature_sampling_window == 0:  # feature_sampling_window divides dead_sampling_window

            feature_sparsity = act_freq_scores / n_frac_active_tokens
            log_feature_sparsity = torch.log10(feature_sparsity +
                                               1e-10).detach().cpu()
                ...
                wandb.log(
                    {
                        ...
                        log_feature_sparsity.mean().item(),
                        "plots/feature_density_line_chart":
                        wandb_histogram,
                    },
        ...
        ghost_grad_neuron_mask = (
            ...
            > sparse_autoencoder.cfg.dead_feature_window).bool()
        ...
        with torch.no_grad():
            ...
            if sparse_autoencoder.cfg.log_to_wandb and (
                (n_training_steps + 1) %
                    sparse_autoencoder.cfg.wandb_log_frequency == 0):
                # metrics for currents acts
                l0 = (feature_acts > 0).float().sum(-1).mean()
                current_learning_rate = optimizer.param_groups[0]["lr"]

                per_token_l2_loss = (sae_out -
                                     sae_in).pow(2).sum(dim=-1).squeeze()
                ...
                wandb.log(
                    {
                        ...
                        mse_loss.item(),
                        "losses/l1_loss":
                        l1_loss.item() / sparse_autoencoder.l1_coefficient
                        if sparse_autoencoder.l1_coefficient > 0 else
                        l1_loss.item(),  # normalize by l1 coefficient
                        ...
                        explained_variance.mean().item(),
                        "metrics/explained_variance_std":
                        explained_variance.std().item(),
                        "metrics/l0":
                        l0.item(),
                        # sparsity
                        "sparsity/mean_passes_since_fired":
                        n_forward_passes_since_fired.mean().item(),
                        "sparsity/n_passes_since_fired_over_threshold":
                        ghost_grad_neuron_mask.sum().item(),
                        "sparsity/below_1e-5":
                        (feature_sparsity < 1e-5).float().mean().item(),
                        "sparsity/below_1e-6":
                        (feature_sparsity < 1e-6).float().mean().item(),
            ...
            if sparse_autoencoder.cfg.log_to_wandb and (
                ...
                run_evals(sparse_autoencoder, activation_store, model,
                          n_training_steps)
                sparse_autoencoder.train()
        ...
        if n_training_steps % 250 == 0:  # todo: make this a config
            cfg = sparse_autoencoder.cfg
            # save sae to checkpoints folder
            path = f"{cfg.checkpoint_path}/{n_training_steps}_{sparse_autoencoder.get_name()}.pt"
